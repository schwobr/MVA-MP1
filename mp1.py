...
def generate_a_triangle(noise=0.0, free_location=False, denoising = False):
    ...
    if denoising: 
        imdata2 = generate_a_drawing(figsize, U, V)
...
def generate_dataset_classification(nb_samples, noise=0.0, free_location=False, denoising= False):
    # Getting im_size:
    ...
    for i in range(nb_samples):
        ...
        if category == 0:
            if denoising:
                X[i], Y[i] = generate_a_rectangle(noise, free_location, denoising)
            else:
                X[i] = generate_a_rectangle(noise, free_location)
        elif category == 1:             
            if denoising:
                X[i], Y[i] = generate_a_disk(noise, free_location, denoising)
            else:
                X[i] = generate_a_disk(noise, free_location)
        else:
            if denoising:
                [X[i], Y[i], _] = generate_a_triangle(noise, free_location, denoising)
            else:
